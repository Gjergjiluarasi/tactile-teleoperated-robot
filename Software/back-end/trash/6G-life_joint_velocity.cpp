...
int main(int argc, char **argv)
{
    ...
    try
    {
        ...
        double time = 0.0;
        robot.control(  [&time, &initial_pose, &message, &leftFR3EndoWrist, &wakeup_time, &ret](const franka::RobotState &robot_state, franka::Duration period) -> franka::CartesianPose
                        {   
                            ////////////////////////////////////////////////////////////////////////////////////////////////
                            // EndoWrist part
                            ...
                            // Franka Emika part                                
                            static std::array<double, 16> new_pose;
                            ...
                            if (leftFR3EndoWrist.recv(message, MAX_SIZE))
                            {
                                ...
                                new_pose[12] += -period.toSec()*std::min(2.0, std::max(-2.0, message[4]));
                                new_pose[13] += -period.toSec()*std::min(2.0, std::max(-2.0, message[5]));
                                new_pose[14] += period.toSec()*std::min(2.0, std::max(-2.0, message[6]));
                            }
                            else
                            {
                                new_pose[12] += 0;
