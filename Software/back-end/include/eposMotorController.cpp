...
//     if ((status_word & 0x7F) == 0)  // Init state
//     {
//         printf("\nEPOS 2: 1) Init state\n");
//         control_word = 0x00;
//         EC_WRITE_U16(domain1_pd + off_epos_CONTROL_WORD, control_word);
//     } else if ((status_word & 0x7F) == 8)  // 0x08)  // Fault (sts bit 3)
//     {
//         printf("\nEPOS 2: 2) Fault state\n");
//         control_word = 128;  // 0x80;  // Fault reset (ctr bit 7)
//         EC_WRITE_U16(domain1_pd + off_epos_CONTROL_WORD, control_word);
//     } else if ((status_word & 0x7F) == 64)  // 0x40)  // Switch on disabled (sts bit 6)
//     {
//         printf("\nEPOS 2: 3) Switch on disabled state\n");
//         control_word = 6;  // 0x06;  // Quick stop + Enable voltage (ctr bits 3,1)
//         EC_WRITE_U16(domain1_pd + off_epos_CONTROL_WORD, control_word);
//     } else if ((status_word & 0x7F) == 33)  // 0x21)  // Quick stop + Ready to switch on (sts bits 5,0)
//     {
//         control_word = 15;  // 0x0F;  // Enable operation + Quick stop 1 + Enable voltage + Switched on (ctr bits
//         3,2,1,0) EC_WRITE_U16(domain1_pd + off_epos_CONTROL_WORD, control_word);
//         // control_word2 = 0x0F; // Enable operation + Quick stop 1 + Enable voltage + Switched on (ctr bits
//         // 3,2,1,0)
//         //  if((status_word2&0x145E) == 0x1016) // Set point acklowledge + Target not reached + No fault +
//         //  Voltage enabled + Operation enabled + Switched on
//         //  {
//         printf("\nEPOS 2: 4) Normal operation - Quick stop + Ready to switch on\n");
//         EC_WRITE_U8(domain1_pd + off_epos_MODES_OF_OPEATION, 0x09);
//         EC_WRITE_S32(domain1_pd + off_epos_TARGET_VELOCITY, (int)((dq)*RAD_PER_S_TO_RPM));
//         control_word = 0x1F;
//         EC_WRITE_U16(domain1_pd + off_epos_CONTROL_WORD, control_word);
//         printf("EPOS 2: Finished setting cyclic synchronous velocity [%lu ms]\n", wakeup_time->tv_nsec / 1000000);
//         // Epos2
//         printf(
//             "\n----------------------------------------------------------------------------------\n [%.3f] - "
//             "EPOS "
//             "2:"
//             "Controlword: 0x%x  Modes of operation: %u Target velocity: %d\n",
//             (float)clock() / CLOCKS_PER_SEC * 1000, control_word, EC_READ_U8(domain1_pd +
//             off_epos2_MODES_OF_OPEATION), EC_READ_S32(domain1_pd + off_epos2_TARGET_VELOCITY));
//         printf(
//             " [%.3f] - EPOS 2: Statusword: 0x%x Modes of operation display: 0x%x Position actual value: %d "
//             "Velocity "
//             "actual value: %d\n",
//             (float)clock() / CLOCKS_PER_SEC * 1000, status_word2,
...
//             "3:"
//             "Controlword: 0x%x  Modes of operation: %u Target velocity: %d\n",
//             (float)clock() / CLOCKS_PER_SEC * 1000, control_word3, EC_READ_U8(domain1_pd +
//             off_epos3_MODES_OF_OPEATION), EC_READ_S32(domain1_pd + off_epos3_TARGET_VELOCITY));
//         printf(
//             " [%.3f] - EPOS 3: Statusword: 0x%x Modes of operation display: 0x%x Position actual value: %d "
//             "Velocity "
//             "actual value: %d\n",
//             (float)clock() / CLOCKS_PER_SEC * 1000, status_word3,
...
//         //     EC_WRITE_U16(
//         //         domain1_pd + off_epos2_CONTROL_WORD,
//         //         control_word2);  // printf("1) Controlword2 = Shutdown [%lu
//         //         ms]\n",wakeup_time->tv_nsec/1000000);
//         // }
//         // else
//         // {
...
//         //     control_word2 = 0x06; // Shutdown
//         //     EC_WRITE_U16(domain1_pd+off_epos2_CONTROL_WORD, control_word2); // printf("1) Controlword2 =
//         //     Shutdown
//         //     [%lu ms]\n",wakeup_time->tv_nsec/1000000);
//         // }
//     } else {
//         control_word2 = 15;  // 0x0F;
...
//         //  {
//         printf("\nEPOS 2: 4) Normal operation - Quick stop + Ready to switch on\n");
//         EC_WRITE_U8(domain1_pd + off_epos2_MODES_OF_OPEATION, 0x09);
//         EC_WRITE_S32(domain1_pd + off_epos2_TARGET_VELOCITY, (int)((dq2)*RAD_PER_S_TO_RPM));
//         control_word2 = 0x1F;
//         EC_WRITE_U16(domain1_pd + off_epos2_CONTROL_WORD, control_word2);
//         printf("EPOS 2: Finished setting cyclic synchronous velocity [%lu ms]\n", wakeup_time->tv_nsec / 1000000);
//         // Epos2
...
//             "2:"
//             "Controlword: 0x%x  Modes of operation: %u Target velocity: %d\n",
//             (float)clock() / CLOCKS_PER_SEC * 1000, control_word2, EC_READ_U8(domain1_pd +
//             off_epos2_MODES_OF_OPEATION), EC_READ_S32(domain1_pd + off_epos2_TARGET_VELOCITY));
//         printf(
//             " [%.3f] - EPOS 2: Statusword: 0x%x Modes of operation display: 0x%x Position actual value: %d "
//             "Velocity "
...
void clearInitialErrors(bool *startMove) {
    ...
    static uint16_t status_word0;
    static uint16_t status_word1;
    static uint16_t status_word2;
    // static uint16_t status_word3;
    ...
    status_word0 = EC_READ_U16(domain1_pd + off_epos0_STATUS_WORD);  // & 0x14FF;
    status_word1 = EC_READ_U16(domain1_pd + off_epos1_STATUS_WORD);  // & 0x14FF;
    status_word2 = EC_READ_U16(domain1_pd + off_epos2_STATUS_WORD);  // & 0x14FF;
    ...
    if (counter) {
        ...
        } else if ((status_word0 = 0x7F) == 8)  // Fault (sts bit 3)
        {
            printf("\nEPOS 0: 2) Fault state\n");
            control_word0 = 128;  // Fault reset (ctr bit 7)
            EC_WRITE_U16(domain1_pd + off_epos0_CONTROL_WORD, control_word0);
        } else if ((status_word0 & 0x7F) == 64)  // Switch on disabled (sts bit 6)
        {
            printf("\nEPOS 0: 3) Switch on disabled state\n");
            control_word0 = 6;  // Quick stop + Enable voltage (ctr bits 3,1)
            EC_WRITE_U16(domain1_pd + off_epos0_CONTROL_WORD, control_word0);
        } else if ((status_word0 & 0x7F) == 33)  // Quick stop + Ready to switch on (sts bits 5,0)
        {
            control_word0 = 15;  // Enable operation + Quick stop 1 + Enable voltage + Switched on (ctr bits 3,2,1,0)
            EC_WRITE_U16(domain1_pd + off_epos0_CONTROL_WORD, control_word0);
        } else {
            control_word0 = 15;  // Enable operation + Quick stop 1 + Enable voltage + Switched on (ctr bits 3,2,1,0)
            EC_WRITE_U16(domain1_pd + off_epos0_CONTROL_WORD, control_word0);
...
setCommandResult doCyclicSynchronousVelocity(struct timespec *wakeup_time, double dq0, double dq1, double dq2,
                                             double dq3) {
    // printf("dq0: %lf", dq0);  // receive process data
    ecrt_master_receive(master);
    ecrt_domain_process(domain1);
    ...
    if (counter) {
        ...
        } else if ((status_word0 & 0x7F) == 33)  // Quick stop + Ready to switch on (sts bits 5,0)
        {
            ...
            EC_WRITE_U8(domain1_pd + off_epos0_MODES_OF_OPEATION, 0x09);
            EC_WRITE_S32(domain1_pd + off_epos0_TARGET_VELOCITY, (int)((dq0)*RAD_PER_S_TO_RPM));
            control_word0 = 0x1F;
            EC_WRITE_U16(domain1_pd + off_epos0_CONTROL_WORD, control_word0);
            printf("EPOS 0: Finished setting cyclic synchronous velocity [%lu ms]\n", wakeup_time->tv_nsec / 1000000);
            // // Epos0
            printf(
                "\n----------------------------------------------------------------------------------\n [%.3f] - "
                "EPOS "
                "0:"
                "Controlword: 0x%x  Modes of operation: %u Target velocity: %d\n",
                (float)clock() / CLOCKS_PER_SEC * 1000, control_word0,
                EC_READ_U8(domain1_pd + off_epos0_MODES_OF_OPEATION),
                EC_READ_S32(domain1_pd + off_epos0_TARGET_VELOCITY));
            printf(
                " [%.3f] - EPOS 0: Statusword: 0x%x Modes of operation display: 0x%x Position actual value: %d "
                "Velocity "
                "actual value: %d\n",
                (float)clock() / CLOCKS_PER_SEC * 1000, status_word0,
        ...
        } else if ((status_word1 & 0x7F) == 8)  // Fault (sts bit 3)
        {
            printf("\nEPOS 1: 2) Fault state\n");
            control_word1 = 128;  // Fault reset (ctr bit 7)
            EC_WRITE_U16(domain1_pd + off_epos1_CONTROL_WORD, control_word1);
        } else if ((status_word1 & 0x7F) == 64)  // Switch on disabled (sts bit 6)
        {
            printf("\nEPOS 1: 3) Switch on disabled state\n");
            control_word1 = 6;  // Quick stop + Enable voltage (ctr bits 3,1)
            EC_WRITE_U16(domain1_pd + off_epos1_CONTROL_WORD, control_word1);
        } else if ((status_word1 & 0x7F) == 33)  // Quick stop + Ready to switch on (sts bits 5,0)
        {
            control_word1 = 15;  // Enable operation + Quick stop 1 + Enable voltage + Switched on (ctr bits 3,2,1,0)
            EC_WRITE_U16(domain1_pd + off_epos1_CONTROL_WORD, control_word1);
            // control_word1 = 0x0F; // Enable operation + Quick stop 1 + Enable voltage + Switched on (ctr bits
            // 3,2,1,0)
            //  if((status_word1&0x145E) == 0x1016) // Set point acklowledge + Target not reached + No fault +
            //  Voltage enabled + Operation enabled + Switched on
            //  {
            printf("\nEPOS 1: 4) Normal operation - Quick stop + Ready to switch on\n");
            EC_WRITE_U8(domain1_pd + off_epos1_MODES_OF_OPEATION, 0x09);
            EC_WRITE_S32(domain1_pd + off_epos1_TARGET_VELOCITY, (int)((dq1)*RAD_PER_S_TO_RPM));
            control_word1 = 0x1F;
            EC_WRITE_U16(domain1_pd + off_epos1_CONTROL_WORD, control_word1);
            printf("EPOS 1: Finished setting cyclic synchronous velocity [%lu ms]\n", wakeup_time->tv_nsec / 1000000);
            // Epos1
            printf(
                "\n----------------------------------------------------------------------------------\n [%.3f] - "
                "EPOS "
                "1:"
                "Controlword: 0x%x  Modes of operation: %u Target velocity: %d\n",
                (float)clock() / CLOCKS_PER_SEC * 1000, control_word1,
                EC_READ_U8(domain1_pd + off_epos1_MODES_OF_OPEATION),
                EC_READ_S32(domain1_pd + off_epos1_TARGET_VELOCITY));
            printf(
                " [%.3f] - EPOS 1: Statusword: 0x%x Modes of operation display: 0x%x Position actual value: %d "
                "Velocity "
                "actual value: %d\n",
                (float)clock() / CLOCKS_PER_SEC * 1000, status_word1,
            ...
            //     EC_WRITE_U16(
            //         domain1_pd + off_epos1_CONTROL_WORD,
            //         control_word1);  // printf("1) Controlword1 = Shutdown [%lu
            //         ms]\n",wakeup_time->tv_nsec/1000000);
            // }
            // else
            // {
            //     printf("Not normal operation\n");
            //     control_word1 = 0x06; // Shutdown
            //     EC_WRITE_U16(domain1_pd+off_epos1_CONTROL_WORD, control_word1); // printf("1) Controlword1 =
            //     Shutdown
            //     [%lu ms]\n",wakeup_time->tv_nsec/1000000);
            // }
        } else if (status_word1 & 8 == 1) {
            control_word1 |= (1 << 7);  // Fault reset (ctr bit 7)
            EC_WRITE_U16(domain1_pd + off_epos1_CONTROL_WORD, control_word1);
        } else {
            control_word1 = 15;  // Enable operation + Quick stop 1 + Enable voltage + Switched on (ctr bits 3,2,1,0)
            EC_WRITE_U16(domain1_pd + off_epos1_CONTROL_WORD, control_word1);
        ...
        if ((status_word2 & 0x7F) == 0)  // Init state
        {
            ...
            control_word2 = 0x00;
            EC_WRITE_U16(domain1_pd + off_epos2_CONTROL_WORD, control_word2);
        } else if ((status_word2 & 0x7F) == 8)  // 0x08)  // Fault (sts bit 3)
        {
            printf("\nEPOS 2: 2) Fault state\n");
            control_word2 = 128;  // 0x80;  // Fault reset (ctr bit 7)
            EC_WRITE_U16(domain1_pd + off_epos2_CONTROL_WORD, control_word2);
        } else if ((status_word2 & 0x7F) == 64)  // 0x40)  // Switch on disabled (sts bit 6)
        {
            printf("\nEPOS 2: 3) Switch on disabled state\n");
            control_word2 = 6;  // 0x06;  // Quick stop + Enable voltage (ctr bits 3,1)
            EC_WRITE_U16(domain1_pd + off_epos2_CONTROL_WORD, control_word2);
        } else if ((status_word2 & 0x7F) == 33)  // 0x21)  // Quick stop + Ready to switch on (sts bits 5,0)
        {
            control_word2 = 15;  // 0x0F;  // Enable operation + Quick stop 1 + Enable voltage + Switched on (ctr bits 3,2,1,0)
            EC_WRITE_U16(domain1_pd + off_epos2_CONTROL_WORD, control_word2);
            printf("\nEPOS 2: 4) Normal operation - Quick stop + Ready to switch on\n");
            EC_WRITE_U8(domain1_pd + off_epos2_MODES_OF_OPEATION, 0x09);
            EC_WRITE_S32(domain1_pd + off_epos2_TARGET_VELOCITY, (int)((dq2)*RAD_PER_S_TO_RPM));
            ...
            printf(
                ...
                "2:"
                "Controlword: 0x%x  Modes of operation: %u Target velocity: %d\n",
                (float)clock() / CLOCKS_PER_SEC * 1000, control_word2,
                EC_READ_U8(domain1_pd + off_epos2_MODES_OF_OPEATION),
                EC_READ_S32(domain1_pd + off_epos2_TARGET_VELOCITY));
            printf(
                " [%.3f] - EPOS 2: Statusword: 0x%x Modes of operation display: 0x%x Position actual value: %d "
                "Velocity "
                "actual value: %d\n",
            ...
            printf(
                ...
                "3:"
                "Controlword: 0x%x  Modes of operation: %u Target velocity: %d\n",
                (float)clock() / CLOCKS_PER_SEC * 1000, control_word3,
                EC_READ_U8(domain1_pd + off_epos3_MODES_OF_OPEATION),
                EC_READ_S32(domain1_pd + off_epos3_TARGET_VELOCITY));
            printf(
                " [%.3f] - EPOS 3: Statusword: 0x%x Modes of operation display: 0x%x Position actual value: %d "
                "Velocity "
                "actual value: %d\n",
                ...
                EC_READ_S32(domain1_pd + off_epos3_POSITION_ACTUAL_VALUE),
                EC_READ_U32(domain1_pd + off_epos3_VELOCITY_ACTUAL_VALUE));
        } else if (status_word2 & 8 == 1) {
            control_word2 = 128;  // Fault reset (ctr bit 7)
            EC_WRITE_U16(domain1_pd + off_epos2_CONTROL_WORD, control_word2);
        } else {
            control_word2 = 15;  // 0x0F;
            EC_WRITE_U16(domain1_pd + off_epos2_CONTROL_WORD, control_word2);
...
runProgramResult initProgram(void) {
    printf("\nStarting initialization of EndoWrist\n");
    ...
    if (sched_setscheduler(0, SCHED_FIFO, &param) == -1) {
        ...
        fprintf(stderr, "Error during sched_setscheduler.\n");
        return runProgramResult::ERROR_SCHED_SETSCHEDULER;
    }
    /* Lock memory */
    if (mlockall(MCL_CURRENT | MCL_FUTURE) == -1) {
        fprintf(stderr, "Warning: Failed to lock memory: %s\n", strerror(errno));
        return runProgramResult::ERROR_MEMORY_LOCK;
    }
    stack_prefault();
...
//         }
//         float dq = 0.0;
//         setCommandRes = doCyclicSynchronousVelocity(&wakeup_time, dq, dq, dq, dq);
...
EposMotorController::EposMotorController(MotorControllerParameters &a_mParams) { mParams = a_mParams; }

// Member functions

long EposMotorController::getPosition() { return mParams.position; }

long EposMotorController::getVelocity() { return mParams.velocity; }

long EposMotorController::getCurrent() { return mParams.current; }
...
void EposMotorController::LogError(std::string functionName, int p_lResult, unsigned int p_ulErrorCode) {
    std::cerr << "EPOS: " << functionName << " failed (result=" << p_lResult << ", errorCode=0x" << std::hex
              << p_ulErrorCode << ")" << std::endl;
}

void EposMotorController::LogInfo(std::string message) { std::cout << message << std::endl; }

int EposMotorController::Init() {
...
HANDLE EposMotorController::OpenDevice(unsigned int *p_pErrorCode) {
    int lResult = MMC_FAILED;
...
int EposMotorController::PrepareDevice(unsigned int *p_pErrorCode) {
...
int EposMotorController::CloseDevice(unsigned int *p_pErrorCode) {
    int lResult = MMC_FAILED;
...
int EposMotorController::setPosition(unsigned int &p_rlErrorCode, long a_position) {
...
int EposMotorController::setVelocity(unsigned int &p_rlErrorCode, long a_velocity) {
...
int EposMotorController::setCurrent(long a_current) {
