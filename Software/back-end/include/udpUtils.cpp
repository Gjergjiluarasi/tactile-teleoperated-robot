...
namespace UdpClientServer
{
     ...
     */
    int UdpClient::send(const void *msg, size_t size)
    {
     ...
     */
    int UdpServer::recv(void *msg, size_t max_size)
    {
     ...
     */
    int UdpServer::timed_recv(void *msg, size_t max_size, int max_wait_ms)
    {
        fd_set s;
        FD_ZERO(&s);
        FD_SET(f_socket, &s);
